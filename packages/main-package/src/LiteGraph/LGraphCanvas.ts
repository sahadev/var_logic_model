...
import { distance, getPropertyPrintableValue, isInsideRectangle, overlapBounding, node_colors } from "./utils";
...
export class LGraphCanvas {
     ...
     * @param {LGraph} graph
     */
    setGraph(graph: LGraph, skip_clear) {
        if (this.graph == graph) {
            return;
        }
    ...
    */
     ...
     */
    setDirty(fgcanvas: bool, bgcanvas: bool) {
        if (fgcanvas) {
            this.dirty_canvas = true;
        }
        if (bgcanvas) {
    ...
    */
        ...
        if (render_title || title_mode == LiteGraph.TRANSPARENT_TITLE) {
            ...
            if (node.onDrawTitleBar) {
                ...
                if (this.use_gradients) {
                    let grad = this.gradients[title_color];
                    if (!grad) {
                        grad = this.gradients[title_color] = ctx.createLinearGradient(0, 0, 400, 0);
    ...
    processNodeWidgets(
        ...
        for (let i = 0; i < node.widgets.length; ++i) {
            ...
            let widget_height = widget.computeSize ? widget.computeSize(width)[1] : LiteGraph.NODE_WIDGET_HEIGHT;
            let widget_width = widget.width || width;
            //outside
            if (widget != active_widget &&
                (x < 6 || x > widget_width - 12 || y < widget.last_y || y > widget.last_y + widget_height || widget.last_y === undefined))
                continue;

            let old_value = widget.value;
    ...
    boundaryNodesForSelection() {
        return this.getBoundaryNodes(Object.values(this.selected_nodes));
    }
     ...
     */
    alignNodes(nodes: LGraphNode[], direction: "top" | "bottom" | "left" | "right", align_to: LGraphNode | null) {
        if (!nodes) {
            return;
        }
        ...
        if (align_to === undefined) {
            boundaryNodes = this.getBoundaryNodes(nodes)
        } else {
            boundaryNodes = {
                "top": align_to,
                "right": align_to,
    ...
    onNodeAlign(value, options, event, prev_menu, node) {
        ...
        function inner_clicked(value) {
            this.alignNodes(activeCanvas.selected_nodes, value.toLowerCase(), node);
        }
    }
    ...
    onGroupAlign(value, options, event, prev_menu) {
        ...
        function inner_clicked(value) {
            this.alignNodes(activeCanvas.selected_nodes, value.toLowerCase());
        }
    }
    ...
    onMenuResizeNode(value, options, e, menu, node) {
        if (!node) {
            return;
        }
    ...
    showConnectionMenu(optPass) { // addNodeMenu for connection
        ...
        function inner_clicked(v, options, e) {
            ...
            switch (v) {
                ...
                default:
                    // check for defaults nodes for this slottype
                    let nodeCreated = that.createDefaultNodeForSlot(Object.assign(opts, {
        ...
        function refreshHelper() {
            ...
            if (that.onSearchBox) {
                ...
                for (let i in LiteGraph.searchbox_extras) {
                    ...
                    if (this.search_limit !== -1 && c++ > this.search_limit) {
                        break;
                    }
                }
    ...
    onMenuNodeCollapse(value, options, e, menu, node) {
        node.graph.beforeChange(/*?*/);
    ...
    onMenuNodePin(value, options, e, menu, node) {
        node.pin();
    };
    ...
    onMenuNodeColors(value, options, e, menu, node) {
        if (!node) {
        ...
        function inner_clicked(v) {
            ...
            let color = v.value ? node_colors[v.value] : null;
    ...
    onMenuNodeShapes(value, options, e, menu, node) {
        if (!node) {
    ...
    onMenuNodeRemove(value, options, e, menu, node) {
        if (!node) {
            throw "no node passed";
        }
    ...
    onMenuNodeClone(value, options, e, menu, node) {
    ...
    getCanvasMenuOptions() {
        ...
        if (this.getMenuOptions) {
            ...
            options = [
                ...
                { content: "Add Group", callback: that.onGroupAdd },
                //{ content: "Arrange", callback: that.graph.arrange },
                { content: "Collapse All", callback: that.onMenuCollapseAll } // 有问题，不能用
            ];
    ...
    getNodeMenuOptions(node) {
            ...
            options = [
                {
                    ...
                    has_submenu: true,
                    disabled: true,
                    callback: this.showMenuNodeOptionalInputs
                },
                {
                    content: "Outputs",
                    has_submenu: true,
                    disabled: true,
                    callback: this.showMenuNodeOptionalOutputs
                },
                null,
                {
                    content: "Properties",
                    has_submenu: true,
                    callback: this.onShowMenuNodeProperties
                },
                null,
                {
                    content: "Title",
                ...
                },
                {
                    content: "Mode",
                    has_submenu: true,
                    callback: this.onMenuNodeMode
                }];
            if (node.resizable !== false) {
                options.push({
                    content: "Resize", callback: this.onMenuResizeNode
                });
            }
            options.push(
                {
                    content: "Collapse",
                    callback: this.onMenuNodeCollapse
                },
                { content: "Pin", callback: this.onMenuNodePin },
                {
                    content: "Colors",
                    has_submenu: true,
                    callback: this.onMenuNodeColors
                },
                {
                    content: "Shapes",
                    has_submenu: true,
                    callback: this.onMenuNodeShapes
                },
                null
            );
        }
        ...
        if (0) //TODO
            options.push({
                content: "To Subgraph",
                callback: this.onMenuNodeToSubgraph
            });
        ...
        options.push(null, {
            ...
            callback: this.onMenuNodeRemove
        });
    ...
    getGroupMenuOptions(node) {
        let o = [
            { content: "Title", callback: this.onShowPropertyEditor },
            {
                content: "Color",
                has_submenu: true,
                callback: this.onMenuNodeColors
            ...
            null,
            { content: "Remove", callback: this.onMenuNodeRemove }
        ];
